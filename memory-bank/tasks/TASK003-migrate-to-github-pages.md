...
- DNS 更新（手动操作）：A 记录指向 GitHub Pages IP，`www` CNAME → `iamsail.github.io`
- Vercel 项目暂停/删除（待 DNS 稳定后）
- 补充缺失翻译：`compare`、`blog`、`faq` 命名空间

## Post-Completion Fix (2026-03-27)
- 将 deploy workflow 从 `peaceiris/actions-gh-pages`（需要 `gh-pages` 分支）
  改为官方 `actions/upload-pages-artifact` + `actions/deploy-pages`，
  与 GitHub Pages Settings 中 "GitHub Actions" Source 模式对齐，无需额外分支。
