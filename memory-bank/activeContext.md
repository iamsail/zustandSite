...
- Created `.github/workflows/deploy.yml` (push to `main` triggers build & deploy via official `actions/deploy-pages`).
...
2. DNS: Add A records to GitHub Pages IPs (`185.199.108.153/154/155/156`) + `www` CNAME → `iamsail.github.io`.
3. After DNS stabilizes, pause/delete Vercel project.
4. Fill in missing translation namespaces: `compare`, `blog`, `faq` in `messages/*.json`.

## Active Decisions
- **Content Structure:** Using `app/[locale]/` for routing.
- **Styling:** Tailwind CSS for rapid UI development.
- **i18n:** `next-intl` static mode (`setRequestLocale` + `generateStaticParams`).
- **Deployment:** GitHub Pages via official `actions/deploy-pages` (GitHub Actions mode), custom domain `zustand.site`.
